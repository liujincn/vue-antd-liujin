    ...
    <a-card :bordered="false"
      ...
      <a-table :columns="columns"
        ...
        <template slot="taskType"
                  slot-scope="text,record">
          <a-tag v-if="record.taskType ===0"
                 color="#2db7f5">实时</a-tag>
          <a-tag v-else>定时</a-tag>
        </template>

        <template slot="operation"
                  slot-scope="text,record">
          <a-button :disabled="record.status === 2"
                    type="danger"
                    style="margin-left: 10px;"
                    @click="cancel(record)">撤销
          </a-button>
          <a-button :disabled="record.status !== 0"
                    type="danger"
                    style="margin-left: 10px;"
        ...
        <template slot="sendStartDate"
          ...
          <div v-else>
            ...
            <br />
            <a-tag color="#2db7f5"
                   style="margin-top: 10px">截止日期：{{ record.sendEndDate }} 23:59:59</a-tag>
          </div>
        </template>
        <template slot="sendTime"
          ...
          <div v-if="record.taskType ===0">&#45;&#45;</div>
          <p v-else
             v-for="(item,index) in record.sendTime"
             :key="index">
            <a-tag>{{item}}</a-tag>
          </p>
        </template>

        <template slot="sendInterval"
                  slot-scope="text,record">
        ...
        <template slot="goodsItems"
                  slot-scope="text,record">
          <div v-if="record.goodsItems.length ===0 ">&#45;&#45;</div>
          <p v-else
             v-for="(item,index) in record.goodsItems"
             :key="index">
            <a-tag color="#2db7f5">
        ...
        <template slot="status"
          ...
          <template v-for="(item,index) in statusData">
            <span :key="index"
                  v-if="record.status === item.id">
              <a-tag :type="item.type">{{ item.name }}</a-tag>
            </span>
          </template>
        ...
        <div slot="expandedRowRender"
          ...
          <a-table :dataSource="record.subTasks"
                   :pagination="false"
                   :rowKey="(record, index) => index">
            ...
            <a-table-column title="发送状态"
              ...
              <template slot-scope="isSent">
                <a-tag v-if="isSent === true"
                       color="green">已发送</a-tag>
                <a-tag v-if="isSent === false"
                       color="#2db7f5">未发送</a-tag>
              </template>
            </a-table-column>
            <a-table-column title="实际发送时间"
                            dataIndex="sendTime"
                            key="sendTime">
          ...
          <a-row style="line-height: 30px; padding: 10px 0; width: 100%"
                 v-if="record.filter.excludes!==null">
            <el-tag v-for="(item,index) in scope.row.filter.excludes"
                    :key="index"
                    style="margin:0 10px 10px; width: 70px;text-align: center">{{item}}
            </el-tag>
          </a-row>
          <a-row style="line-height: 30px; padding: 10px 0;"
                 v-if="record.filter.receivers!==null">
            <a-tag color="green"
                   v-for="(item,index) in record.filter.receivers"
                   :key="index"
                   style="margin:0 10px 10px; width: 70px; text-align: center">{{item}}
            </a-tag>
          </a-row>
