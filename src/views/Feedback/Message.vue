<template>
  <div class="app-container">
          ...
          <a-row style="float: left">
            <a-form-item class="input-item">
              <a-dropdown>
                <a-menu slot="overlay"
                        @click="postFreeze">
                  <a-menu-item v-for="item in freezeTypes"
                               :key="item.id">
                    <a-tag :color="item.tagType"
                           :title="item.title">{{item.type}}</a-tag>
                    {{item.label}}
                  </a-menu-item>
            ...
            <a-form-item class="input-item">
              <a-dropdown>
                <a-menu slot="overlay"
                        @click="postMute">
                  <a-menu-item v-for="item in muteTypes"
                               :key="item.id">
                    {{item.label}}
                  </a-menu-item>
                  <a-menu-item key="other">其他</a-menu-item>
                </a-menu>
                <a-button type="primary">禁言操作
                  <a-icon type="down" />
                </a-button>
              </a-dropdown>
            </a-form-item>
    ...
    <a-modal title="请输入你需要封号的天数"
             v-model="dayFormVisible"
             :footer="null"
             width="400px">
      <a-form :form="form"
              @submit="submitOtherDay">
        <a-form-item label="天数"
                     :label-col="{ span: 6 }"
                     :wrapper-col="{ span: 18 }">
          <a-input @change="controlMute"
                   v-decorator="['days', { rules: [{ required: true, message: '请输入时间' }] }]"></a-input>
        </a-form-item>
        <a-form-item label="封号类别"
                     :label-col="{ span: 6 }"
                     :wrapper-col="{ span: 18 }">
          <a-select v-decorator="['freezeType',{ rules: [{ required: true, message: '请选择封号类别' }] },]"
                    placeholder="请选择封号类别">
            <a-select-option v-for="(item,index) in types"
                             :key="index"
        ...
        <a-button type="primary"
                  html-type="submit"
                  style="width:100%">提交</a-button>
      </a-form>
    </a-modal>

    <a-modal title="请输入你需要禁言的时间"
             v-model="muniteFormVisible"
             :footer="null"
             width="400px">
      <a-form :form="form"
              @submit="submitOtherMunite">
        <a-form-item label="分钟"
                     :label-col="{ span: 6 }"
                     :wrapper-col="{ span: 18 }">
          <a-input @change="controlFreeze"
                   v-decorator="['otherMunite', { rules: [{ required: true, message: '请输入时间' }] }]"></a-input>
        </a-form-item>
        <a-button type="primary"
                  html-type="submit"
                  style="width:100%">提交</a-button>
