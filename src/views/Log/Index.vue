        ...
        <a-form-model ref="ruleForm"
          ...
          <a-row>
            ...
            <a-form-model-item class="input">
              <single-selector @select="selectRoleType"
                               v-model="queryType"></single-selector>
            </a-form-model-item>

            <a-form-model-item class="input-item"
                               :style="{marginLeft:'10px'}">
              <a-select v-model="form.goodsType"
                        placeholder="物品类型"
                        allowClear>
                <a-select-option v-for="item in goodsTypes"
                                 :key="item.id"
                                 :value="item.id">
                  {{item.name}}
                </a-select-option>
            ...
            <a-form-model-item class="input-item"
              ...
              <a-select v-model="form.sType"
                        placeholder="请选择物品类型"
                        @change="changeQueryTypes">

                <a-select-option v-for="item in queryTypes"
                                 :key="item.id"
            ...
            <a-form-model-item class="input-item">
              <template v-for="(el, index) in queryTypes">
                <a-input @input="handlesTypeValue"
                         allowClear
                         :key="index"
          ...
          <a-row>
            <a-form-model-item class="input-item">
              <a-select v-model="form.oprType"
                        clearable
                        placeholder="操作类型"
                        @change="changeOprType">
                <a-select-option v-for="(item,index) in dataType"
                                 :key="index"
                                 :value="item.id">{{item.name}}
                </a-select-option>
              </a-select>
            ...
            <a-form-model-item class="input-item">
              <a-select :disabled="isDisabled"
                        ...
                        placeholder="请选择查询类型"
                        allowClear>
                <a-select-option v-for="(item,index) in numricalResource"
                                 :key="index"
                                 :value="item.id">{{item.name}}
                </a-select-option>
              </a-select>
            ...
            <a-form-model-item class="input">
              <date-picker @startTime="getStartTime"
                           @endTime="getEndTime"></date-picker>
            </a-form-model-item>
            <a-form-model-item class="input">
