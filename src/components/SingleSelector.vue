<template>
    <div>
        ...
        <template v-for="(item, index) in data">
            <a-input class="input-item" :style="{ marginRight: '0px'}" :key="item.id" v-if="item.id === selectedId"
                     v-model="selectedTxt" @input="handleInput" :placeholder="'请输入'+ data[index].name"
                     allowClear></a-input>
